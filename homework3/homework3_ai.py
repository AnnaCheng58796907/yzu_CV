...
def bgr_cmyk(img: np.ndarray) -> tuple[np.ndarray,
                                       np.ndarray,
                                       np.ndarray,
                                       np.ndarray]:
    """
...
def draw_roc_flag(width: int) -> np.ndarray:
    ...
    # 創建紅地圖像
    red_field_cmyk = np.full((height, width, 4),
                             [c_red, m_red, y_red, k_red],
                             dtype=np.float32)
    flag_image = cmyk_to_bgr(red_field_cmyk[..., 0], red_field_cmyk[..., 1],
                             red_field_cmyk[..., 2], red_field_cmyk[..., 3])

    # 創建青天圖像 (長方形)
    blue_canton_cmyk = np.full((canton_height, canton_width, 4),
                               [c_blue, m_blue, y_blue, k_blue],
                               dtype=np.float32)
    blue_canton = cmyk_to_bgr(blue_canton_cmyk[..., 0],
                              blue_canton_cmyk[..., 1],
                              blue_canton_cmyk[..., 2],
                              blue_canton_cmyk[..., 3])
    ...
    for i in range(num_rays):
        ...
        current_angle_p = np.deg2rad(current_angle)
        current_angle_p2 = np.deg2rad(current_angle + ray_angle_half)
        current_angle_p3 = np.deg2rad(current_angle - ray_angle_half)

        # 計算光芒尖端點
        tip_x = int(center_x + ray_tip_length * np.cos(current_angle_p))
        tip_y = int(center_y - ray_tip_length * np.sin(current_angle_p))

        # 計算光芒基部所在的兩個點 (位於白日體圓周上)
        base1_x = int(center_x + white_sun_radius * np.cos(current_angle_p2))
        base1_y = int(center_y - white_sun_radius * np.sin(current_angle_p2))

        base2_x = int(center_x + white_sun_radius * np.cos(current_angle_p3))
        base2_y = int(center_y - white_sun_radius * np.sin(current_angle_p3))

        # 創建頂點陣列，並繪製填充多邊形（三角形）
        ray_points = np.array([(tip_x, tip_y),
                               (base1_x, base1_y),
                               (base2_x, base2_y)])
    ...
    cv2.circle(blue_canton,
               (center_x, center_y),
               white_sun_radius + blue_ring_width,
               (int(blue_color_bgr[0]),
                int(blue_color_bgr[1]),
                int(blue_color_bgr[2])),
               blue_ring_width)
